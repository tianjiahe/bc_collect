...
void CRealGraphView::drawRealCur()
{
   ...
   PEnset(m_hPE,  PEP_nSUBSETS,     1   );									 // 设置子集的个数，定义对象包含的子集数
...
*/
   ...
   //PEvsetEx( m_hPE, PEP_dwaSUBSETCOLORS, 0, 3, dwArray, 0 );//此属性阵列控制的颜色被用来区分多个信息子集之间的物体的图像。
   
   //int nLineTypes[] = { PELT_MEDIUMSOLID, PELT_MEDIUMSOLID, PELT_MEDIUMSOLID};
   //PEvset(m_hPE, PEP_naSUBSETLINETYPES, nLineTypes, 4);   //此属性阵列控制线类型用于区分多个信息子集之间的物体的图像。
  
   //int nPointTypes[] = { PEPT_DOTSOLID, PEPT_UPTRIANGLESOLID, PEPT_SQUARESOLID};//PEPT_DOWNTRIANGLESOLID };
   //PEvset(m_hPE, PEP_naSUBSETPOINTTYPES, nPointTypes,4);//此属性阵列控制点类型用于区分多个信息子集之间的物体的图像。
   ...
   PEnset(m_hPE, PEP_bFIXEDFONTS,             TRUE );      // 使字体保持设置尺寸时的尺寸变化图。
   ...
   PEnset(m_hPE, PEP_bSIMPLEPOINTLEGEND,      TRUE );      // 此属性可以控制点的图例符号样式
   PEnset(m_hPE, PEP_bSIMPLELINELEGEND,       TRUE );      // 此属性可以控制线路图例符号样式。

   PEnset(m_hPE, PEP_nGRIDLINECONTROL,  PEGLC_BOTH );        // 绘制的网格的水平和垂直线，PEGLC_NONE-

   PEnset(m_hPE, PEP_nPLOTTINGMETHOD,  PEGPM_POINTSPLUSSPLINE ); // 绘制坐标点的方式  点+曲线，PEGPM_SPLINE-曲线形式 ，
...
void CRealGraphView::OnTimer(UINT nIDEvent) 
{
	...
	{   
		...
		PEvset(m_hPE, PEP_faAPPENDYDATA, &newy, 1);        // 追加新的数据，坐标延伸方式 0-对数，1-线性，2 指数
		PEvset(m_hPE, PEP_faAPPENDXDATA, &newx, 1);        
		//PEvset(m_hPE, PEP_faAPPENDYDATA, &maxy, 1);      // 追加新的坐标数据
		//PEvset(m_hPE, PEP_faAPPENDXDATA, &newx, 1); 
		//PEvset(m_hPE, PEP_faAPPENDYDATA, &miny, 1);      // 追加新的坐标数据
		//PEvset(m_hPE, PEP_faAPPENDXDATA, &newx, 1); 
...
// 				PEvsetcellEx(m_hPE, PEP_faXDATA, 2, m_nRealTimeCounter, &newx);
			...
			{
				PEnset(m_hPE, PEP_nMANUALSCALECONTROLX, PEMSC_NONE);  // X轴动态跟踪坐标点。Automatic Scale Control
			}
			...
			::InvalidateRect(m_hPE, NULL, FALSE);    // 通知重画
		}
		else
		{
			////**********************************************

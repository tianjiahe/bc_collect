...
void CProtocol::SendFrame()
{
	ZeroMemory(pCom->m_txbuf, TX_BUF_LEN );
    memcpy( pCom->m_txbuf, m_bFrame, m_bFrameLen);
	pCom->WriteToPort( (char *)pCom->m_txbuf, m_bFrameLen);
}

// 计算校验和
BYTE CProtocol::checkSum()
{
    WORD  total = 0;
...
void CProtocol::FormatTx(CString &str)
{
...
void CProtocol::FormatRx(CString &str)
{
   BYTE *pbuf = pCom->m_rxbuf;
   str.Format("%02x %02x %02x %02x %02x %02x %02x %02x %02x %02x %02x",
